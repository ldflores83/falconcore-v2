...
class GoogleDriveProvider {
    ...
    async findOrCreateFolder(folderName, projectId, accessToken, refreshToken, parentFolderId) {
        const oauth2Client = new googleapis_1.google.auth.OAuth2();
        oauth2Client.setCredentials({
    ...
    async uploadFileWithTokens(folderId, filename, content, mimeType, accessToken, refreshToken) {
        const oauth2Client = new googleapis_1.google.auth.OAuth2();
        oauth2Client.setCredentials({
            access_token: accessToken,
            refresh_token: refreshToken
    ...
    async createEmptyDocument(filename) {
        const auth = await this.getAuthenticatedClient('default');
        const docs = googleapis_1.google.docs({ version: 'v1', auth });
        const drive = googleapis_1.google.drive({ version: 'v3', auth });
    ...
    async replaceDocumentContent(documentId, content) {
        const auth = await this.getAuthenticatedClient('default');
        const docs = googleapis_1.google.docs({ version: 'v1', auth });
