...
exports.validateFileUpload = exports.API_CONFIG = void 0;
exports.API_CONFIG = {
    // Límites de archivos
...
const validateFileUpload = (files) => {
    ...
    if (files.length > exports.API_CONFIG.MAX_FILES_PER_UPLOAD) {
        errors.push(`Maximum ${exports.API_CONFIG.MAX_FILES_PER_UPLOAD} files allowed per upload`);
    }
    // Verificar tamaño total
    const totalSize = files.reduce((sum, file) => sum + file.size, 0);
    if (totalSize > exports.API_CONFIG.MAX_FILE_SIZE) {
        errors.push(`Total file size (${Math.round(totalSize / 1024 / 1024 * 100) / 100}MB) exceeds ${exports.API_CONFIG.MAX_FILE_SIZE / 1024 / 1024}MB limit`);
    }
    // Verificar tipos MIME
    for (const file of files) {
        if (!exports.API_CONFIG.ALLOWED_MIME_TYPES.includes(file.mimeType)) {
