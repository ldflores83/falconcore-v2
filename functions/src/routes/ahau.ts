...
ahauRouter.post('/public/drafts/create', verifyApiKey, async (req, res) => {
  try {
...
ahauRouter.get('/public/drafts/list', verifyApiKey, async (req, res) => {
  try {
    const { tenant } = (req as any);
    ...
    const draftsSnapshot = await query.limit(parseInt(limit as string)).get();
    const drafts = draftsSnapshot.docs.map(doc => ({
      id: doc.id,
...
ahauRouter.post('/public/publish', verifyApiKey, async (req, res) => {
  try {
    const { tenant } = (req as any);
    const { draftId, isCompanyPage = false } = req.body;
    const db = getDb();
